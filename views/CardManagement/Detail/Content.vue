<!-- 物联卡查看 -->
<template>
  <a-row :gutter="[24, 24]" v-if="cardId">
    <a-col :span="24">
      <a-card>
        <a-descriptions size="small" :column="3" bordered>
          ...
          <a-descriptions-item :label="$t('Detail.index.427958-3')">
            <template v-if="detail.deviceName">
              <a @click="onDeviceClick">{{ detail.deviceName }}</a>
            </template>
            <template v-else>
              --
            </template>
          ...
          <a-descriptions-item :label="$t('Detail.index.427958-32')">
            <template v-if="detail.poolAlias || detail.poolName">
...
const onDeviceClick = () => {
  menuStore.jumpPage('device/Instance/Detail', {
    params: {id: detail.value.deviceId}
  })
}
